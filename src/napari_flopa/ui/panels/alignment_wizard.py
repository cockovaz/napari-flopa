...
class AlignmentWizard(QDialog):
    ...
    def _build_ui(self):
        ...
        form.addRow("Shift (µs):", self._shift_spin)
        ...
        self._start_spin.valueChanged.connect(self._sync_shift_from_delays)
        self._stop_spin.valueChanged.connect(self._sync_shift_from_delays)
        self._shift_spin.valueChanged.connect(self._sync_stop_from_shift)
    ...
    def _on_optimize_result(self, result):
        ...
        # translates the whole acceptance window rather than replacing it.
        self._syncing = True
        self._start_spin.setValue(self._start_spin.value() + best_us)
        ...
        self._status.info(f"Applied {best_us:+.3f} µs to both delays{note}.")

    @Slot(object)
    def _on_profiles_result(self, result):
        forward, backward = result
        self._alignment_plot.set_labels(
            "Pixel", "Photon count", "Click Show align"
        )
        self._show_profiles(
            np.arange(len(forward), dtype=float),
    ...
    def _show_profiles(self, x_values, labelled):
        series = [
